...
<head>
    ...
    <style>
        ...
        .details{background:#fff;padding:12px;border-radius:6px;border:1px solid #eef2f7;margin-top:10px}
...
<body>
    <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
            ...
            <div class="muted">Recordatorio</div>
        </div>

        <h3>Hola {{ $client->name }},</h3>
        <p class="muted">Te recordamos que tienes una cita <strong>hoy</strong>.</p>

        <div class="details">
            <p><strong>Servicio:</strong> {{ $appointment->service->name ?? '—' }}</p>
            <p><strong>Barbero:</strong> {{ $appointment->barber->name ?? '—' }}</p>
            <p><strong>Hora:</strong> {{ \Carbon\Carbon::parse($appointment->appointment_time)->format('g:i A') }}</p>
        </div>

        <p style="margin-top:12px">Adjuntamos tu pase digital en PDF para presentar el día de la cita.</p>
